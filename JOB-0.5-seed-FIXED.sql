...
insert into public.payment_orders (order_no, student_id, payer_uid, lane, kind, amount, discount, method, status, payer_note, created_at)
select 'PO-TEST-AWAIT', s.id, s.id, 'self', 'offer_letter', 3000, 0, 'offline', 'awaiting_review', '【测试】待核销订单', now()
...
insert into public.payment_orders (order_no, student_id, payer_uid, lane, kind, amount, discount, method, status, reject_reason, payer_note, created_at)
select 'PO-TEST-REJECT', s.id, s.id, 'self', 'concierge', 600, 0, 'offline', 'rejected', '【测试】付款凭证模糊，请重传', '【测试】已驳回订单', now()
  from public.students s
 order by s.created_at desc
 limit 1
 on conflict (order_no) do nothing;
